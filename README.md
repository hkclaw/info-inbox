...
- Dump a block of text. Exact-duplicate text (trim, not fuzzy) warns 「呢段同已有 note 一樣」 and needs 仍然存入. Dump also accepts files (txt/md/csv/json/html/log/pdf/xlsx/xls/docx/doc/pptx/ppt/rtf/odt locally; other types still become a note with 「未支援抽文字」). Images stay filename-only. Ollama llama3.2 at http://127.0.0.1:11434 writes tags + a one-line summary; if it is down the note stays, tags stay empty, and the page shows 「連唔到 Ollama（127.0.0.1:11434）」. No invented tags.
