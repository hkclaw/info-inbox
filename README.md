...
- Notes list. Click a tag to filter; 「清篩選／顯示全部」 clears. 編輯 PATCHes text only and re-embeds that note with local nomic if it is pulled (same path as Dump/backfill); if nomic is missing the save still succeeds, the stale vector is dropped, Search stays keyword-only, and it does not pretend 連唔到 while 11434 is up. Save does not auto-reclassify. 刪除 is DELETE /api/notes/:id. Empty tags or classifyError show 「重新分類」.
